...
async function Draw(){
  ...
  oX=0, oY=0, oZ=Math.min(150, Math.max(30, (.3-C(t/2))*400))
  Rl=S(t/8)/3, Pt=Math.min(Math.PI/2,Math.max(-.5,(.3-C(t))*3)), Yw=C(t/8)*4
  ...
  if(!((t*60|0)%(30+S(t/2)*30|0))) mode = !mode

  x.globalAlpha = 1
  sd2 = 50

  pt = Math.max(0,Math.min(Math.PI,(.3-C(t))*4))
  for(m=sd2;m--;){
    ls = (3+m*6/2)**1
    sd = ((6+m*4)/2)|0
    for(i=sd;i--;){
      X1 = X = S(p=Math.PI*2/sd*i+Math.PI+t/2) * ls
      Z1 = Z = C(p) * ls
      Y = pt?0:Math.min(10,Math.max(-10,((100+C(Math.atan2(X1,Z1)*2-t*10)*100)-100)/3))
      R(0,pt,0)
      R(0,0,Math.PI*2/sd2*m*S(t))
      R(Rl,Pt,Yw,1)
      if(Z>0){
        s = Math.min(5e3, (3000+70*ls)/Z)
        if((x.globalAlpha = 1/(1+ls**7/1e13))>.025){
          l = Q()
          if(mode){
            x.drawImage(burst,l[0]-s/2,l[1]-s/2,s,s)
          } else {
            s*=2
            x.drawImage(starImgs[1+(5+(Math.atan2(X1,Z1)*2.25+t*20)|0)%8].img,l[0]-s/2/1.05,l[1]-s/2/1.05,s,s)
          }
        }
      }
    }
