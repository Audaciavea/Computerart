  ...
  if(!t){
    ...
    stroke = (scol, fcol, lwo=1, od=true, oga=1) => {
      if(scol){
        ...
        x.strokeStyle = scol
        x.lineWidth = Math.min(400,100*lwo/Z)
        if(od) x.stroke()
        x.lineWidth /= 4
        x.globalAlpha = 1*oga
        x.stroke()
    ...
    doRecurse = (theta=0) => {
      ...
      B = []
      a=[]
      recurse = (tx, ty, tz, ls, theta, depth, j) => {
        if(ls < 4) return
        for(let i=sd;i--;) {
          if(depth>1){
            X1 = S(p=Math.PI*2/sd*i+theta)*ls
            Y1 = ty
            Z1 = C(p)*ls

            nextTheta = (theta)*(depth+.25)
            nextTy = (-length/2+(j-1)+.5)*sp
            X2 = S(p=Math.PI*2/sd*i+nextTheta)*ls
            Y2 = nextTy
            Z2 = C(p)*ls
            p1 = p
            p2 = Math.acos((Y2-Y1)/(Math.hypot(X1,Y2-Y1,Z1)+.1))
            X = 0
            Y = 0
            Z = ls
            R(0,0,p1)
            ...
            X = tx + S(p=Math.PI*2/sd*i+theta)*ls
            Y = ty
            Z = tz + C(p)*ls            
          }

          a = [...a, [X, Y, Z, depth, j]]
          recurse(X,Y,Z,ls/2.5, (theta)*(depth+.25), depth+.25, j)
        }
      }
      for(let j = 0; j<length; j++){
        a = []
        recurse(0,(-length/2+j+.5)*sp,0, ls, theta+Math.PI*2/length*j*2, 1, j)
        B = [...B, a]
      }
    }
  }

  oX=0, oY=0, oZ=Math.min(300,Math.max(100,(.3-S(t/2))*300))
  Rl=Math.min(Math.PI*.5,Math.max(0,(.3-C(t/4))*3)), Pt=-t/4, Yw=S(t/4)*6
  ...
  for(j=0;j<B[0].length;j++){
    ...
    for(i=0; i<B.length; ++i){
      ...
      if(Z>0){
        l = Q()
        x.lineTo(...l)
        s = Math.min(1e4, 1e7/Z/((1+B[i][j][3])**6.5*5))
        idx = 1+((2+(B[i][j][3]*9+(t*200/360*8))|0)%8)
        x.drawImage(starImgs[idx].img,l[0]-s/2/1.05,l[1]-s/2/1.05,s,s)
        
      }
    }
    col1 = `hsla(${360/1.7*B[i-1][j][3]-50+t*200},99%,50%,.33)`
    Z=oZ
    stroke(col1,'',3e4/((1+B[i-1][j][3])**6*5), false)
  }
