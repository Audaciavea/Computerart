...
async function Draw(){
  ...
  if(!t){
    ...
    rw = 40
    rl=pt=yw=0
    tubeSegs = Array(rw*cl).fill().map(v=>[])
    loadTubes = () =>{
      sp = 1
      //rl=-C(t/1.5)*3, pt=-C(t/2)*8, yw=-t*4
      rl=S(t/4)*10, pt=C(t/4)*10, yw+=Math.min(30,Math.max(-5,(.3-C(t/2))*100))/100
      grid = []
      Array(rw*cl).fill().map((v,i) => {
        ls = sp/2 * ((2*(sp**2))**.5)/2
        X = ((i%cl)-cl/2+.5)*ls*3 + ((i/cl|0)%2?0:ls*1.5)
        Y = (((i/cl|0)%rw)-rw/2+.5) * ((.75*sp)**.5)/2
        Z = 0
        R(Math.PI/2,0,0)
        tx = X
        ty = Y
        ...
        a = []
        sd = 6
        vls = (.1+.9*(.5+S(t*24+grid.length-1)/2))*1
        for(i=sd;i--;){
          X = tx + S(p=Math.PI*2/sd*i+Math.PI/sd) * ls * 1.25*vls
          Y = ty + C(p) * ls * 1.25*vls
          Z = tz + 0
          a = [...a, [X,Y,Z]]
        }
        if((d=Math.hypot(tx,ty))<4 && (d>2.5)){
        //if((d=Math.hypot(tx,ty))<2.85 && (d>2.7) && ty>-1.7){
          grid = [...grid, a]
          tubeSegs[grid.length-1] = [...tubeSegs[grid.length-1], [a.map(v=>{
            X = v[0]
            Y = v[1]
            Z = v[2]
            R(rl,pt,yw)
  ...
  oX=0, oY=0, oZ=Math.min(8,Math.max(2,(.3+S(t/2))*20))
  Rl=C(t)/3, Pt=-t/4, Yw=t/4
  ...
  grid.map(v=>{
    ...
    if(1 && Z>0){
      ...
      s*=1.25
      x.drawImage(starImgs[2].img,l[0]-s/2/1.05,l[1]-s/2/1.05,s,s)
    }
  })

  
  tubeSegs = tubeSegs.map((tube, idx) => {
    tube.map((v, i)=>{
      if(i){
        l1 = i
        l2 = i-1
        for(m=sd;m--;){
          ...
          if(Z>0)x.lineTo(...Q())
          al = (v[1]/2)**2
          col1 = ''//al>.1?`hsla(${360/tube.length*i/4 + t*100 + (v[2]?180:0)},99%,${50+50**v[1]}%,${al})`:''
          col2 = `hsla(${360/tube.length*i/3 + t*50 + (v[2]?180:0)},99%,${40+80**v[1]**.5}%,${v[1]*.3})`
          x.closePath()
