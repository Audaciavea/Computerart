...
async function Draw(){
  if(!t){
    ...
    shpLs = 1
    base_shp = subDividedCube(shpLs, 3, 2)
    a = []
    base_shp.map(v=>{
      tx=ty=tz=0
      v.map(q=>{
      ...
      if(d<=shpLs*.98) a = [...a, JSON.parse(JSON.stringify(v.map(q=>{
        X = q[0]
        Y = q[1]
        Z = q[2]
        d = Math.hypot(X,Y,Z)
        fac = d>=shpLs/1.125?(d+.111111111111111)**8.5-.1111111111111111:d
        q[0] /= d
        q[1] /= d
        q[2] /= d
        q[0] *= fac
        ...
        q[2] *= fac
        maxLs = 1.5625
        if(q[0]>maxLs)q[0] = maxLs
        if(q[1]>maxLs)q[1] = maxLs
        if(q[2]>maxLs)q[2] = maxLs
        if(q[0]<-maxLs)q[0] = -maxLs
        if(q[1]<-maxLs)q[1] = -maxLs
        if(q[2]<-maxLs)q[2] = -maxLs
        if(q[0]==maxLs || q[0]==-maxLs){
    ...
    cl = 3
    br = 3
    sectors = Array(rw*cl*br).fill().map((v, i)=>{
      X = ((i%cl)-cl/2+.5)*sp
      Y = (((i/cl|0)%rw)-rw/2+.5)*sp
      Z = ((i/cl/rw|0)-br/2+.5)*sp
      return [X,Y,Z]
    ...
    curdir = Rn()*6|0
    nav = () => {
      for(let m=3;m--;) oldcell[m]=curcell[m]
      curcell[Rn()*3|0]+=Rn()<.5?sp:-sp
    }
    nav()
  }
  ...
  Rl+=.0025
  speed = C(t/5)/20
  vx = (curcell[0]-oldcell[0]) * speed
  vy = (curcell[1]-oldcell[1]) * speed
  vz = (curcell[2]-oldcell[2]) * speed
  //p1 = -(Math.atan2(vx,vz))
  //p2 = -(Math.PI/2+Math.acos(vy/(Math.hypot(vx,vy,vz)+.001)))
  //Pt += (p2-Pt)/20
  //Yw += (p1-Yw)/20
  oX -= vx
  oY -= vy
  ...
  //if(Math.hypot(curcell[0]+oX,curcell[1]+oY,curcell[2]+oZ)<speed*2) nav()
  ...
  B.map(n=>{
    ...
    ty = n[1]
    tz = n[2]
    if(tx+oX < -sp*1.5) n[0] += sp*3
    if(tx+oX > sp*1.5) n[0] -= sp*3
    if(ty+oY < -sp*1.5) n[1] += sp*3
    if(ty+oY > sp*1.5) n[1] -= sp*3
    if(tz+oZ < -sp*1.5) n[2] += sp*3
    if(tz+oZ > sp*1.5) n[2] -= sp*3
    ...
    R(Rl,Pt,Yw,1)
    if(Z>0){
      s = Math.min(5e4, 500/Z)
      l = Q()
      x.globalAlpha = 1/(1+Math.hypot(X,Y,Z)**8/9999)
    ...
    base_shp.map(v=>{
      ...
      v.map(q=>{
        ...
        Z = q[2] + tz
        R(Rl, Pt, Yw, 1)
        if(Z>0 && (.1/(1+Math.hypot(X,Y,Z)**8/9999))>.025) x.lineTo(...Q())
      })
      alpha = .1/(1+Math.hypot(X,Y,Z)**8/9999)
      col1 = `hsla(${t*100},99%,50%,${alpha})`
      col2 = col1
      if(alpha>.025) stroke(col1, col2,.2,true)
    })
  })
  ...
  sectors.map(n=>{
    ...
    for(let m=8;m--;){
      ...
      alpha = 1/(1+Math.hypot(tx+oX,ty+oY,tz+oZ)**8/2000)
      if(alpha > .1){
        ...
        if(Z>0){
          ...
          l=Q()
          x.font = (20/Z) + 'px courier'
          x.fillText(`X:${Math.round(X1/sp)}`, l[0], l[1]-12/1.5/Z)
          x.fillText(`Y:${Math.round(Y1/sp)}`, l[0], l[1]+8/1.5/Z)
          x.fillText(`Z:${Math.round(Z1/sp)}`, l[0], l[1]+28/1.5/Z)
        }
      }
    }    
  })
