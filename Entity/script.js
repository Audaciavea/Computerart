...
async function Draw(){
  ...
  if(!t){
    ...
    iSpheresc = 3
    spheres = Array(iSpheresc).fill().map((v,i)=>{
      X = 0
      Y = 0
      Z = 0
      ls = (4+i*10)*2
      CT = 16+i*10|0
      shp = JSON.parse(JSON.stringify(geoSphere(0, 0, 0, CT, ls)[4])).map(q=>{
        q[3]=[]
        return q
      })
      return [X, Y, Z, shp]
    ...
    centris = subDividedCube(5,2,10)
  }
  
  oX=0, oY=0, oZ=Math.min(100,Math.max(30,(.3+S(t/2))*200))
  Rl=t/8, Pt=-.2+S(t/3)/2, Yw=C(t/4)*4
    ...
    v[3].map((q,j)=>{
      ...
      if(!i){
        if(!((t*60|0)%pFreq)) {  
          ...
          vy = Y/d*iPv
          vz = Z/d*iPv
          q[3] = [...q[3], [X, Y, Z, vx, vy, vz, 1]]
        }

        q[3] = q[3].filter(v=>v[6]>.1)
        q[3].map((k,g)=>{
          X1 = k[0]+=k[3]
          Y1 = k[1]+=k[4]
          Z1 = k[2]+=k[5]
          d1 = Math.hypot(X1,Y1,Z1)
          vx=vy=vz=0
          spheres.map((n,m)=>{
            tx = n[0]
            ty = n[1]
          ...
          if(g){
            ...
            R(Rl,Pt,Yw,1)
            if(Z>0) x.lineTo(...Q())
            stroke(`hsla(${-S(t)*3000+360/q[3].length*g},99%,${50+50/q[3].length*g}%,.5)`,'',k[6]*50)
          }
          X = k[0]
          Y = k[1]
          Z = k[2]
          ...
          if(0&&Z>0){
            l = Q()
            s = Math.min(3e3,3e3/Z*k[6])
            x.fillStyle = `#00ff0004`
            x.fillRect(l[0]-s/2,l[1]-s/2,s,s)
            s/=3
            x.fillStyle = `#00ffff15`
            x.fillRect(l[0]-s/2,l[1]-s/2,s,s)
            s/=3
      ...
      if(1&&Z>0){
        l = Q()
        s = Math.min(3e3,5e3/Z)
        x.fillStyle = `hsla(${-t*150+360/2/spheres.length*i},99%,50%,.025)`
        x.fillRect(l[0]-s/2,l[1]-s/2,s,s)
        s/=3
        x.fillStyle = `hsla(${-t*150+360/2/spheres.length*i},99%,50%,.15)`
        x.fillRect(l[0]-s/2,l[1]-s/2,s,s)
        s/=3
        x.fillStyle = `#ffffffff`
        x.fillRect(l[0]-s/2,l[1]-s/2,s,s)
