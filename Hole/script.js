...
async function Draw(){
  ...
  spd = 0//Math.min(6,Math.max(-1,(.3-C(t/2))*20))
  oX=0, oY=0, oZ=Math.min(150,Math.max(18,50+C(t*2)*200))
  //Rl=C(t/16)/4, Pt=C(t/8)/4, Yw=S(t/8)/4
  p=(1+Math.PI*2/(br1*sp3)*zpos)/2
  //Rl=-S(p-.25)/2, Pt=C(t*4*spd)/16, Yw=S(p)/2
  Rl=C(t/16)/4,Pt=Math.min(Math.PI/2,Math.max(.08,(.3+C(t))*5)),Yw=C(t/2)*8
  ...
  base_shps.map(shp=>{
    ...
    base_shp.map((v,i)=>{
      ...
      col1 = `hsla(${i+t*2000},99%,${500+C(t)*450}%,${alpha1/2})`
      col2 = `hsla(${i+t*2000},99%,${500+C(t)*450}%,${alpha2*2})`
