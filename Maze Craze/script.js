...
async function Draw(){
  ...
  if(!t){
    ...
    genMaze = (w, h, sp=1) => {
      ...
      for(let j=0; j<h; j++){
        for(let i=0; i<w; i++){
          ...
          Y = (j-h/2+.5)*sp
          Z = 0
          a =[...a, [X,Y,Z,[], !i || !j || i==w-1 || j==h-1, idx]]
        }
      }

      for(let i=0; i<w-1; i++){
        a[i][4] = a[i+1][4] = true
        a[i+w*(h-1)][4] = a[i+1+w*(h-1)][4] = true
        a[i][3] = [...a[i][3], i+1]
        a[i+w*(h-1)][3] = [...a[i+w*(h-1)][3], i+1+w*(h-1)]
      }
      for(let i=0; i<h-1; i++){
        a[i*w][4] = a[i+1][4] = true
        a[i*w+w-1][4] = a[i*w+w+w-1][4] = true
        a[i*w][3] = [...a[i*w][3], i*w+w]
        a[i*w+w-1][3] = [...a[i*w+w-1][3], i*w+w+w-1]
      }
    ...
    mazeProxy = idx => {
      return genMaze(w=mazew, h=mazeh, sp).map((v, i) => {
        X = v[0]
        Y = v[1]
        Z = v[2]
        lx = ly = lz = 0
    ...
    mazes = Array(6).fill().map((maze, idx) => mazeProxy(idx))
    
    runLen = 1
    growMaze = (maze, idx_=1) => {
      for(let k=(60*(1+idx_))|0; k--;){
        placed = false
        l = maze.filter(v=>v[3].length>0)
        v = l[l.length*Rn()|0]
        idx = v[5]
        for(m=6;!placed && m--;){
          let dir = Rn()*4|0
          for(j=runLen+Rn()*runLen|0;j--;){
            X = idx%w
            Y = idx/w|0
            switch(dir){
              case 0: X--; break
            ...
            if(X>=0 && X<=w-1 && Y>=0 && Y<=h-1){
              tidx = X+Y*w
              if(tidx>=0 && tidx < w*h && !maze[tidx][3].length && !maze[tidx][4]){
                maze[tidx][4] = true
                maze[tidx][3] = [...maze[tidx][3], idx]
                placed = true
                idx = tidx
              }
            }
  ...
  if(1||!done){
    mazes = mazes.map((maze, idx) => {
      if(maze.filter(v=>!v[4]).length){
        growMaze(maze, idx)
      }else{
        maze = mazeProxy(idx)
      }
    ...
    //if(!mazes.filter(maze=>maze.filter(v=>!v[4]).length).length) done = true
    //ip = 0
    //Yw=0//t/8
  }else{
  }
  ip = (1.6+S(pt*3)*4.5)/2.5
  Yw+=C(pt/2) / 64
  ip1 = ip
  ip2 = 1-ip1
  
  pt+=done?1/60:0
  ofd = Math.max(-.0166,(3.6-C(pt) * 4)/10)
  
  oX=0, oY=0, oZ=Math.min(80, Math.max(0,(.3+C(t/2))*100))
  Rl=0, Pt=-C(pt/2)
  ...
  mazes.map(maze=>{
    ...
    maze.map((v, i) => {
      ...
      v[3].map((q, j) => {
        ...
        R(Rl,Pt,Yw,1)
        if(Z>0) x.lineTo(...Q())
          d_ = Math.hypot(nx-v[6],ny-v[7],nz-v[8])
          col = `hsla(${d_**4/500-t*1600},99%,${50+d_**3/999}%,.75)`
        stroke(col,'',4, false)
      })
    })
  })
